...
def reply_actions(
    *, episode_id: int | None = None, turn_idx: int | None = None
    ...
    сообщении, а клавиатура на весь экран убивает ощущение переписки.

    `🇷🇺 Перевод` появляется только когда сказано, какую реплику переводить. Без
    пары адрес неизвестен — так вызывают из-под сообщений самого бота, которые
    и без перевода по-русски.
    """
    builder = InlineKeyboardBuilder()
    builder.button(text="🔍 Разбор", callback_data=EpisodeAction(action="analysis"))
    builder.button(text="🤷 Не знаю", callback_data=EpisodeAction(action="dunno"))
