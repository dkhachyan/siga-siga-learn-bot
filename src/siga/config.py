...
class Settings(BaseSettings):
    ...
    #: Кого пускать в бота — номера Telegram через запятую. **Пусто значит
    #: «никого»**, а не «всех»: наоборот было ровно один день, и этого хватило,
    #: чтобы бот в проде оказался открыт — `.env` там завели из `.env.example`
    #: с пустой строкой. Забыть настройку легко, прочитать предупреждение в
    #: журнале — нет, поэтому ошибка должна запирать, а не отпирать.
    #:
