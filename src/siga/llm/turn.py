...
- Тип эпизода `T` — разговор без целевых слов. Список целевых слов будет \
пуст: оценки не нужны, верни `word_assessments` и `unused_target_words` \
пустыми, а ошибки исправляй рекастом и веди разговор дальше.
